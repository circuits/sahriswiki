...
function loadPage(name) {
    ...
    $.getJSON("/wiki/" + name, function(data) {
        ...
        if (data.success) {
            creole.parse(page[0], data.page.text);
            ...
            $("#status").empty().append(TEMPLATES.PageMeta, {
                    ...
                    rev: data.meta.rev,
                    date: prettyDate(data.meta.date * 1000)});
        } else {
...
function loadMenu() {
    $.getJSON("/wiki/SiteMenu", function(data) {
        var menu = $("#menu");
        menu.empty();
        creole.parse(menu[0], data.page.text);
...
function doEdit() {
    ...
    $.getJSON("/wiki/" + pagename, function(data) {
        ...
        $("#editor textarea").val("");
        if (data.success) {
            $("#editor #text textarea").val(data.page.text);
        } else {
...
function main() {
    $("body").load("/templates/base.xhtml", {}, function() {
